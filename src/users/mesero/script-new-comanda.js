...
async function load_tables() {
    try {
        const respuesta = await fetch('api-tables.php');
        const resultado = await respuesta.json();

        if (!resultado.success) return;

        const select = document.getElementById('choose-table');
        select.innerHTML = `<option value="0">Selecciona la mesa</option>` + resultado.tables.map(mesa => `
            <option value="${mesa.n_table}" ${mesa.ocupada ? 'disabled' : ''}>
                Mesa ${mesa.n_table} ${mesa.ocupada ? '(Ocupada)' : ''}
            </option>
        `).join('');
...
document.addEventListener('change', (e) => {
    if (e.target?.id === 'type-delivery') {
        ...
        if (valueoption == 'mesa') {
            load_tables();
            document.getElementById('input-select-mesa').style.display = 'grid';
            //document.getElementById('name-client').value = "";
        } else if (valueoption == 'domicilio') {
            document.getElementById('input-select-mesa').style.display = 'none';
            document.getElementById('choose-table').value = 0;
        }
    }
});
